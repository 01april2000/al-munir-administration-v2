...
import { Plus, Users, Loader2, Trash2, GraduationCap, Upload, Search } from "lucide-react";
...
export default function SantriManagementPage() {
  ...
  const fetchSantri = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/santri?jenisSantri=PONDOK");
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Exclude kelas from formData for pondok santri
      const { kelas, ...formDataWithoutKelas } = formData;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify(formDataWithoutKelas),
      });
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Manajemen Santri Pondok</h1>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <Input
            type="text"
            placeholder="Cari santri (NIS, nama, asrama, wali, email...)"
            value={searchQuery}
