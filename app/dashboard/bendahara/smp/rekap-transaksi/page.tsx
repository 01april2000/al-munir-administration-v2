...
// SMP-specific kelas options
const KELAS_SMP_OPTIONS = [
...
function RekapTransaksiSMPContent() {
  ...
  const fetchRekap = React.useCallback(async () => {
    ...
    try {
      ...
      // API auto-filters by BENDAHARA_SMP role -> SMP transactions only
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      "download",
      `rekap-transaksi-smp-${selectedKelas || "semua"}-${new Date().toISOString().split("T")[0]}.csv`
    )
    link.click()
  ...
  const getTitle = () => {
    if (!selectedKelas) return "Rekap Semua Transaksi SMP"
    const kelasLabel = KELAS_SMP_OPTIONS.find((k) => k.value === selectedKelas)?.label || selectedKelas
    return `Rekap Transaksi SMP - ${kelasLabel}`
  }
  ...
  if (loading) {
    return <LoadingState message="Memuat rekap transaksi SMP..." />
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Rekap semua transaksi SMP berdasarkan filter yang dipilih
          </p>
        </div>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <div className="w-full md:w-[200px]">
              ...
              <Select value={selectedJenis || "all"} onValueChange={handleJenisChange}>
                ...
                <SelectContent>
                  {JENIS_TRANSAKSI_SMP_OPTIONS.map((option) => (
                    <SelectItem key={option.value || "all"} value={option.value || "all"}>
                      {option.label}
                    </SelectItem>
                  ))}
      ...
      {!error && transaksi.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Daftar Transaksi SMP</CardTitle>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-muted-foreground text-center">
              Tidak ada transaksi SMP yang ditemukan dengan filter yang dipilih.
            </p>
          </CardContent>
        </Card>
      )}
...
export default function RekapTransaksiSMPPage() {
  return (
    <Suspense fallback={<LoadingState message="Memuat rekap transaksi SMP..." />}>
      <RekapTransaksiSMPContent />
    </Suspense>
  )
}
