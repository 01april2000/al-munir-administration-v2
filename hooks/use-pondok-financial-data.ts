...
// Allowed transaction types for Pondok
const ALLOWED_JENIS = ['UANG_SAKU', 'LAUNDRY']

interface UsePondokFinancialDataReturn {
...
export function usePondokFinancialData(
  selectedMonth: number,
  selectedYear: number
): UsePondokFinancialDataReturn {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch all transaksi from all jenis santri (SMK, SMP, PONDOK)
      // Use allJenisSantri=true to bypass role-based jenisSantri filter
      const response = await fetch('/api/transaksi?limit=1000&allJenisSantri=true')
      if (!response.ok) throw new Error('Failed to fetch data')

      const data = await response.json()
      const allTransaksi = data.items || []

      // Filter only UANG_SAKU and LAUNDRY transactions
      const transaksi = allTransaksi.filter((t: RecentTransaksi) => 
        ALLOWED_JENIS.includes(t.jenis)
      )
